...
def is_subtuple_2(tupA, tupB):
    ...
    return False
nb_points = 2
Aglobal = np.zeros((3*len(elems), 2*nb_points*len(conts)))
row = 0
for key_e, value_e in elems.items():
    ...
    for key_c, value_c in conts.items():
        ...
        if is_subtuple_2(key_c, key_e):# or key_c == (key_e[-1]+key_e[0]):
            ...
            Alocal = np.concatenate((Alocal_1, Alocal_2), axis = 1)
            Aglobal[row:row+3, col:col+nb_points*2] = Alocal
            ...
            Aglobal[row:row+3, col:col+nb_points*2] = np.zeros((3,nb_points*2))
        col+=nb_points*2
    row+=3
...
#global Y matrix
Y = np.zeros((3*nb_points*len(conts), 2*nb_points*len(conts)))
yunit = np.matrix([
    [1, -mu],
    [-1, -mu],
    [0,1]
...
def main():
    ...
    with mosek.Env() as env:
        ...
        with env.Task(0, 0) as task:
            ...
            for key, value in conts.items():
                for i in range(nb_points):
                    bkc.extend([mosek.boundkey.up,
                        mosek.boundkey.up,
                        mosek.boundkey.lo])
                    #bkc.append(mosek.boundkey.up)
                    blc.extend([-inf, -inf, 0])
                    buc.extend([0.0, 0.0, +inf])    
            ...
            c = []
            for key, value in conts.items():
                for i in range(2*nb_points):#2variables(t,n)*4nodes*2contact faces
                    c.append(0.0)
            c.append(1.0)
            ...
            asub = []
            aval = []
            for col in range(len(conts)*2*nb_points):
                col_index = []
                col_value = []
                ...
                for row in range(3*nb_points*len(conts)):
                    if Y[row][col] != 0:
