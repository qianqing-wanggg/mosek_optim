...
for i in range(0,nb_course*2+1):
    ...
    nodes[4*i+3] = [0, (nb_course-i-1)*unit_height]
    nodes[4*i+4] = [unit_length, (nb_course-i-1)*unit_height]

#define elements:

elems = dict()#Fx
for i in range(0,nb_course):
    elems[(3*i+1, 3*i+2, 3*i+4, 3*i+3, 3*i+1)] = [1]

#define contacts:
mu = 0.7
conts = dict()#ground or not, t,n
for i in range(0,nb_course):
    conts[(3*i+4, 3*i+3, 3*i+6, 3*i+5)] = [1, [1,0], [0,1]]
...
def plot_elements(elems, title = 'initial elements'):
    ...
    for key, value in elems.items():
        center_x.append(value[-1][0])
        center_y.append(value[-1][1])
        boundary_points = []
        for k in key:
...
def update_node_coor(elems, disps=[]):
    ...
    for key, value in elems.items():
        ...
        trans_x = disps[3*d]
        trans_y = disps[3*d+1]
        new_center = [center[0]+trans_x, center[1]+trans_y]
        rot = disps[3*d+2]       
        boundary_points = []
        #translate all the boundary point of the element
        node_index = 0
        for k in key:
            ...
            node_x = nodes[k][0]-center[0]
            node_y = nodes[k][1]-center[1]
            new_x = node_x*math.cos(rot)-node_y*math.sin(rot)+new_center[0]
            new_y = node_x*math.sin(rot)+node_y*math.cos(rot)+new_center[1]
            nodes[k] = [new_x,new_y]
            node_index+=1
